...
int group = IO_GROUP; /* first group is the input-output group */
int calls[NO_OF_GROUPS] = {0, 0, 0}; /* each group is initialized with 0 calls */
int proportions[NO_OF_GROUPS] = {1, 2, 4}; /* proportions of how many times IO, CALC and INTER groups are called */
struct proc *last_run[NO_OF_GROUPS] = {NIL_PROC, NIL_PROC, NIL_PROC}; /* last process run in each group is initialized with NIL_PROC */
...
/* A process has blocked. */
  ...
  if (!isservp(rp) && !istaskp(rp)){
    if (last_run[rp->group] == rp){ /* checking if the process being removed from the queue isn't in the last_run array */
      last_run[rp->group] = NIL_PROC;
    }
  }

}

PRIVATE void cycle_queue() /* helper function for sched() which moves the user queue by one */
{
...
PRIVATE void sched()
{
  ...
  calls[group]++;   /* incrementing number of calls of the last run group */
  if (calls[group] >= proportions[group]){ /* checking if the last run group hasn't reached its limit of calls */
    calls[group] = 0;
    group = (group + 1)%NO_OF_GROUPS;
  }


  for (i = 0; i < NO_OF_GROUPS; i++){ /* going through all of the groups if there aren't any processes in the current group */
    rp = rdy_head[USER_Q]; /* helper variable set to last run process in general (could be from other group than the current) */

    cycle_queue();

    while(rdy_head[USER_Q] != rp && rdy_head[USER_Q] != last_run[group]){ /* going through the queue until process from the last_run from current group is found  */
      cycle_queue();
    }
    rp = rdy_head[USER_Q]; /* helper variable set to the process from the last_run of current group */
    cycle_queue();
    while(rdy_head[USER_Q] != rp && rdy_head[USER_Q]->group != group){ /* looking for a process in the current group other than the last run process */
      cycle_queue();
    }
    /* if any other process from the group hasn't been found the last run process will be chosen */
    if(rdy_head[USER_Q]->group == group){ /* checking if there are any processes from the group left in the queue */
      break;
    }
    else{ /* if there aren't the next group is chosen */
      group = (group + 1)%NO_OF_GROUPS;
    }
  }
  /* setting last_run process of current group as the chosen process to be run */
  last_run[group] = rdy_head[USER_Q];
  /* incrementing the counter of process calls */
  rdy_head[USER_Q]->times_called++;
  /* running the process */
  pick_proc();
}
