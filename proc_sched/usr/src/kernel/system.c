...
FORWARD _PROTOTYPE( int do_timescalled, (message *m_ptr) );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
      ...
      case SYS_TIMESCALLED: r = do_timescalled(&m); break;
	    default:		r = E_BAD_FCN;
	}
...
/* Handle sys_fork().  m_ptr->PROC1 has forked.  The child is m_ptr->PROC2. */
  ...
  *rpc = *rpp;			/* copy 'proc' struct */
  ...
  rpc->child_utime = 0;
  rpc->child_stime = 0;
  rpc->group = rpp->group; /* child process has the same group as parent */
  rpc->times_called = 0; /* child process has been called 0 times in sched() */
  return(OK);
}
...
PRIVATE int do_timescalled(m_ptr)
message *m_ptr;
{
  struct proc *pp;
  for (pp= BEG_PROC_ADDR; pp<END_PROC_ADDR; pp++) {
    if(pp->p_pid == m_ptr->m1_i1){
      m_ptr->m1_i1 = pp->group;
      m_ptr->m1_i2 = pp->times_called;
      return 0;
    }
  }
  return -1;
